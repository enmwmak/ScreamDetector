package edu.polyu.hazardalert.mfcc;
...
public class MFCC {
	...
	public double compLogEnergy(float[] x) {
		double e = 0.0;
		int N = x.length;
		for (int n=0; n<N; n++) {
			e += Math.pow(x[n],2);
	...
	private void hamming(float[] x) {
		int N = x.length;
		for (int n = 0; n < N; n++) {
			x[n] = (float)((0.54-0.46*Math.cos(2*Math.PI*n/N))*x[n]);
		}
	}
